...
void display::displayNote() {
    if (showTimerNote == true) {
        cout << endl << " Timer note:" << endl << TEXT_BG_SLATE
        << TEXT_FG_YELLW << "  " << note << " " << TEXT_RESET << endl << endl;
    }
}
...
void display::timerExpired() {
    ...
    // Jump to timer reset function to get reset or quit option from user
    resetTimer();
}

// Displays timer paused message and handles resume or quit
///////////////////////////////////////////////////////////
void display::timerPaused()
{
    cout << "   Timer paused..." << endl << endl;

    // Jump to timer retart function to get restart or quit option from user
    restartTimer();
}
